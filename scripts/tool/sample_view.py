...
def process_action(index, df, current_data_dir, folder_name, mode="move"):
    ...
    try:
        ...
        if os.path.exists(src_img_path):
            if mode == "move":
                shutil.move(src_img_path, target_path)
            else:
        ...
        if os.path.exists(src_mask_path):
            ...
            if mode == "move":
                shutil.move(src_mask_path, mask_target_path)
            else:
                shutil.copy2(src_mask_path, mask_target_path)
...
with gr.Blocks(title="Xiaoan 数据分拣系统 Pro", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Column(scale=1):
            ...
            with gr.Accordion("自定义路径", open=True):
                folder_selector = gr.Dropdown(
                    choices=get_available_sources(), label="目标文件夹", allow_custom_value=True
                )
                with gr.Row():
    ...
    # --- 基础导航 ---
    btn_next.click(
        lambda idx, df, d: get_ui_update(idx + 1, df, d), [current_index, global_df, active_data_dir], ui_outputs
    )
    btn_prev.click(
        lambda idx, df, d: get_ui_update(idx - 1, df, d), [current_index, global_df, active_data_dir], ui_outputs
    )
    ...
    # --- 快捷动作 ---
    btn_fast_yes.click(
        lambda idx, df, d: process_action(idx, df, d, "yes_val", "move"),
        [current_index, global_df, active_data_dir],
        ui_outputs + [global_df],
    )

    btn_fast_no.click(
        lambda idx, df, d: process_action(idx, df, d, "no_val", "move"),
        [current_index, global_df, active_data_dir],
        ui_outputs + [global_df],
    )

    btn_trash.click(
        lambda idx, df, d: process_action(idx, df, d, "trash", "move"),
        [current_index, global_df, active_data_dir],
        ui_outputs + [global_df],
    )

    # --- 自定义动作 ---
    btn_custom_move.click(
        lambda idx, df, d, f: process_action(idx, df, d, f, "move"),
        [current_index, global_df, active_data_dir, folder_selector],
        ui_outputs + [global_df],
    )

    btn_custom_copy.click(
        lambda idx, df, d, f: process_action(idx, df, d, f, "copy"),
        [current_index, global_df, active_data_dir, folder_selector],
        ui_outputs + [global_df],
    )
    ...
    btn_yes.click(
        lambda idx, df, d: update_gt(idx, df, d, "yes"), [current_index, global_df, active_data_dir], ui_outputs
    )
    btn_no.click(
        lambda idx, df, d: update_gt(idx, df, d, "no"), [current_index, global_df, active_data_dir], ui_outputs
    )
