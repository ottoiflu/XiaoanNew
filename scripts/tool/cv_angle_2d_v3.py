"""2D图像PCA覆盖angle v3: medium=不合规时不覆盖(保留N/A), 去掉max_input_size。"""
...
n_cv=0; n_skip_medium=0; n_fail=0; t0=time.time()
for i, r in enumerate(rows):
    ...
    # 关键修正:medium=不合规时跳过(保留VLM的[N/A])
    vlm_medium = r.get("medium_status","")
    if vlm_medium.startswith("[不合规"):
        n_skip_medium += 1
        continue
    ...
    try:
        ...
        W,H=pil.size
        result=seg.predict(img_bytes, visual=False, retina_masks=True)
        ...
        n_fail+=1
    if (i+1)%200==0:
        print(f"  {i+1}/{len(rows)} CV覆盖{n_cv} 跳过medium不合规{n_skip_medium} 失败{n_fail} ({time.time()-t0:.0f}s)")

print(f"\nCV覆盖: {n_cv}, 跳过medium不合规: {n_skip_medium}, 失败: {n_fail}, 耗时{time.time()-t0:.0f}s")
print(f"覆盖后: {calc_stats(rows)}")
print(f"覆盖后 angle: {dim_acc(rows,'angle','angle_status')}")

# 额外:排除N/A后的准确率
na_count = sum(1 for r in rows if gt.get(r.get("id",""),{}).get("angle","")=="[N/A]")
non_na_rows = [r for r in rows if gt.get(r.get("id",""),{}).get("angle","")!="[N/A]"]
c = sum(1 for r in non_na_rows if r.get("angle_status","")==gt.get(r.get("id",""),{}).get("angle","") and r.get("angle_status","") and r.get("angle_status","") not in ("err","parse_fail"))
t = sum(1 for r in non_na_rows if r.get("angle_status","") and r.get("angle_status","") not in ("err","parse_fail"))
print(f"排除N/A({na_count}张)后 angle: {c}/{t}={c/t if t else 0:.3f}")

with open("/root/otto/XiaoanNew/outputs/benchmark_output/v4/baseline_qwen36/results_cv_2d_v3.csv","w",newline="",encoding="utf-8") as f:
    w=csv.DictWriter(f,fieldnames=fieldnames)
    w.writeheader(); w.writerows(rows)
print("已保存 results_cv_2d_v3.csv")
