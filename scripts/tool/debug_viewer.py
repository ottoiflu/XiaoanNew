...
MASK_DIR = r"/root/XiaoanNew/yolov8seg_visuals"
...
def load_data():
    ...
    try:
        ...
        if missing:
            return pd.DataFrame(), pd.DataFrame(), f"CSV 缺少必要的列: {missing}\n当前识别到的列名为: {list(df.columns)}"
        ...
        def get_status(row):
            if row['norm_gt'] == row['norm_pred']: return "Correct (预测正确)"
            if "yes" in row['norm_gt'] and "no" in row['norm_pred']: return "FN (漏报 - 实际合规但判违规)"
            if "no" in row['norm_gt'] and "yes" in row['norm_pred']: return "FP (误报 - 实际违规但判合规)"
            return "Mismatch"
...
with gr.Blocks(css=css, title="VLM 结果分析工具") as demo:
    ...
    output_components = [
        current_index, image_display, mask_display, image_info, 
        error_type_display, gt_display, pred_display, reason_display, status_text
    ]

    def handle_search(q):
        if not q: return 0, *get_details(display_df, 0)
    ...
    def navigate(idx, step):
        if display_df.empty: return 0, *get_details(display_df, 0)
