...
class YOLOv8SegInference:
    ...
    def predict(self, 
                source: Union[str, np.ndarray, Image.Image, bytes],
                conf: float = None,
                iou: float = 0.7,
                imgsz: int = 640) -> Dict:
        """
    ...
    def _draw_visualization(self, 
                           img: np.ndarray, 
                           objects: List[Dict],
                           masks_combined: np.ndarray) -> np.ndarray:
        """
        绘制可视化结果
    ...
    def predict_memory(self, image_bytes: bytes) -> io.BytesIO:
        ...
        for obj in result["objects"]:
            ...
            draw.rectangle(bbox, outline=color, width=3)
            draw.text((bbox[0], bbox[1]), f"{label}: {conf:.2f}", 
    ...
    def predict_static_json(self, image_bytes: bytes) -> Dict:
        ...
        for obj in result["objects"]:
            ...
            draw.text((bbox[0] + 5, bbox[1] + 5), f"{label} {conf:.2f}", 
                     fill=(255, 255, 255, 255), font=font)
