...
```python
# 选取距画面中心最近的 Electric bike 作为主车辆（nearest-center 策略）
main_bike_mask = ...
...
以上数值以结构化 JSON 注入 VLM Prompt，作为辅助参考。多车场景下主车辆选取采用 nearest-center 策略（选取距画面中心最近的电动车），对应客户端引导框中心对齐逻辑，详见 XiaoAnEbike 端鲁棒性设计记录。
...
判定规则：
  - final_score ≥ 阈值（优化版 0.35）→ 合规
  - final_score < 阈值 → 违规
  - 构图得分 = 0（门控触发）→ 直接判违规，不计算加权分

分数映射（以 scoring_optimized_cv_p4.yaml 为例）：
  构图：[合规]=1.0，[基本合规]=0.7，不合规=0.0
  角度：[合规]=1.0，[基本合规-角度]=0.5，[不合规-角度]=0.0
  距离：[完全合规]=1.0，[基本合规-压线]=0.5，[不合规-超界]=0.0
  环境：[合规]=1.0，[基本合规-环境]=0.5，[不合规-环境]=0.0
```
