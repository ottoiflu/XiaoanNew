...
4. **步骤 C - CV+VLM 联合判断**：将原始图片、线框轮廓可视化图和结构化 CV 数据注入 Qwen-VL-30B（Prompt `cv_enhanced_p5`），获取四维度状态标签，经 `ScoringEngine` 加权评分得出合规结论。评分权重与阈值由 YAML 配置（当前优化版 `scoring_optimized_cv_p4.yaml`：阈值 0.35）。若 VLM 不可用则降级为规则判断。
5. **步骤 D - 证据保存**：将原图保存至对应合规/违规目录。

**四维度评分规则**（以 `scoring_optimized_cv_p4.yaml` 为例）：

| 维度 | 权重 | 合规状态 | 得分 |
|------|------|----------|------|
| 构图 | 5% | `[合规]` / `[基本合规]` | 1.0 / 0.7 |
| 角度 | 20% | `[合规]` / `[基本合规-角度]` | 1.0 / 0.5 |
| 距离 | 45% | `[完全合规]` / `[基本合规-压线]` / `[不合规-超界]` | 1.0 / 0.5 / 0.0 |
| 环境 | 30% | `[合规]` / `[基本合规-环境]` / `[不合规-环境]` | 1.0 / 0.5 / 0.0 |

综合分数 = Σ(维度权重 × 维度得分)，分数 ≥ 阈值（优化版 0.35）则判定合规；构图门控触发（画面质量不合格）时直接否决。
